    ...
    @foreach($saleDetails as $list)
        ...
        <div class="about_invoice">
          <div class="invoice_details">
            ...
            <p class="order_number order_methodname">Type :
              {{ isset($list['pickup']) ? $list['pickup'] : "-" }}
            </p>
            <p class="order_datetime">
              {{ isset($list['bill_date']) ? Carbon\Carbon::parse($list['bill_date'])->format('d-m-Y h:i A') : "-" }}
            </p>
          </div>
        </div>
      ...
      <div class="customer-details ">
        <ul>
          ...
          <li>
            <label>M :</label>{{ isset($list['mobile_numbr']) ? $list['mobile_numbr'] :"-"}}
          </li>
          <li>
            <label>ADDRESS :</label>{{ isset($list['customer_address']) ? $list['customer_address'] :"-"}}
          </li>
          <li>
            <label>Doctor Name :</label>{{ isset($list['doctor_name']) ? $list['doctor_name'] :"-"}}
          </li>
        </ul>
      </div>
      <div class="items-section">
        ...
        <div class="items-list" style="min-height:295px; position: relative;">
          ...
          @if(isset($list['sales_item']))
            <?php $i = 1; ?>
            @foreach($list['sales_item'] as $listData)
      ...
      <div class="footer">
        <div class="terms-conditions">
          @if(!empty($list['loyalti_point']))
          <p style="border-bottom: 1px solid #000; margin-bottom: 3px;">Loyalti Point : {{ $list['loyalti_point'] ?? '0' }}</p>
          @endif
        ...
        <div class="gst-bifurcation">
          ...
          <p>
            <span>Total MRP</span>
            <label>
              {{isset($list['total_amount']) ? $list['total_amount'] :"₹0.0"}}
            </label>
          </p>
        </div>
        <div class="net-payable" style="border-left: 1px solid #000;">
          <p class="net-amount">
            <label>Net Amount</label>
            <label>
              {{isset($list['total_amount']) ? 'Rs. ' . $list['total_amount'] :"₹0.0"}}
            </label>
          </p>
		  <p class="net-amount">
            <label>Discount</label>
            <label>{{ isset($list['discount_amount']) ? 'Rs. ' . $list['discount_amount'] : "Rs. 0.0" }}</label>
          </p>
          <p class="net-amount">
            <label>Net Amount</label>
          ...
          <ul>
            <li style="display: flex; align-items: center;">
              <img src="{{ isset($list['logo']) ? $list['logo'] :'https://pharma247.in/public/landing_design/images/logo.png'}}"
                alt="Pharma247" title="Phamra247" class="footer-logo" />Pharma247
            </li>
          </ul>
        </div> -->
      </div>

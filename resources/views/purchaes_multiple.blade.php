...
<body class="A5-print-page">
  <div class="invoice-container">
    ...
    @foreach($purchesDetails as $listDetails)
    <div class="container">
      <div class="header">
        <div class="about_pharmacy">
          @if(!empty($listDetails['logo']))
          <div class="logo">
            <img src="{{ isset($listDetails['logo']) ? $listDetails['logo'] : '' }}" border="0"
              alt="Chemist Name" style="width: 70px; height:70px" />
          </div>
          @endif
          <div class="pharmacy_details">
            <p class="pharmacy_name">
              {{ isset($listDetails['user_name']) ? $listDetails['user_name'] :"-"}}
            </p>
            <p>
            </p>
            <p class="address" style="margin-top: 6px;">
              {{ isset($listDetails['address']) ? $listDetails['address'] :"-"}}
            </p>
            <p class="Mobile" style="font-weight: 400; font-size: 12px; margin-top: 6px;">
              <b>M.</b> {{ isset($listDetails['phone_number']) ? $listDetails['phone_number'] :"-"}}
            </p>
          </div>
        </div>
          ...
          <div class="invoice_details">
            <p class="label">
              ...
              <label class="bill_number" style="margin-top: 3px;">Sr No. :
                {{ isset($listDetails['id']) ? $listDetails['id'] :"-"}}
              </label>
              <label class="bill_number" style="margin-top: 3px;">Bill No :
                {{ isset($listDetails['bill_no']) ? $listDetails['bill_no'] :"-"}}
              </label>
            </p>
            <p class="order_number order_methodname" style="margin-top: 3px;">Payment :
              {{ isset($listDetails['payment_type']) ? $listDetails['payment_type'] :"-"}}
            </p>
            <p class="order_datetime" style="margin-top: 3px;">
              Bill Date : {{ isset($listDetails['bill_date']) ? Carbon\Carbon::parse($listDetails['bill_date'])->format('d-m-Y') :"-"}}
            </p>
          </div>
        </div>
      </div>
      <div class="customer-details">
        <ul>
          <li>
            <label>LICENSE 20 :</label>{{ isset($listDetails['license_20']) ? $listDetails['license_20'] : "-" }}
          </li>
          <li>
            <label>LICENSE 21 :</label>{{ isset($listDetails['license_21']) ? $listDetails['license_21'] : "-" }}
          </li>
          <li>
            <label>FSSAI :</label>{{ isset($listDetails['fssai_no']) ? $listDetails['fssai_no'] : "-" }}
          </li>
          <li>
            <label>GSTIN :</label>{{ isset($listDetails['gst_pan']) ? $listDetails['gst_pan'] : "-" }}
          </li>
        </ul>
      </div>
      <div class="customer-details">
        <ul>
          <li>
            <label>Distributor Name :</label>{{ isset($listDetails['distributor_name']) ? $listDetails['distributor_name'] : "-" }}
          </li>
          <li>
            <label>M :</label>{{ isset($listDetails['distributor_phone_number']) ? $listDetails['distributor_phone_number'] : "-" }}
          </li>
          <li>
            <label>GST :</label>{{ isset($listDetails['distributor_gst']) ? $listDetails['distributor_gst'] :"-"}}
          </li>
        </ul>
      </div>
      <div class="items-section">
        ...
        <div class="items-list" style="min-height:295px; position: relative;">
          ...
          <div class="invoice-bg-image">
            <img src="{{ isset($listDetails['logo']) ? $listDetails['logo'] : '' }}" style="border-radius: 50%; height: 150px; opacity: inherit;" alt="Background Logo">
          </div>
		  @endif
          @if(isset($listDetails['item_list']))
          <?php $i = 1; ?>
          @foreach($listDetails['item_list'] as $listData)
      ...
      <div class="footer">
        <div class="gst-bifurcation" style="border-right: 1px solid black; width: 160px;">
          <p>
            <span>Total Qty</span>
            <label>
              {{isset($listDetails['total_item_qty']) ? $listDetails['total_item_qty'] :"₹0.0"}} + {{isset($listDetails['total_item_free_qty']) ? $listDetails['total_item_free_qty'] :"₹0.0"}}
            </label>
          </p>
          <p>
            <span>Total Item(s)</span>
            <label>
              {{isset($listDetails['iteam_count']) ? $listDetails['iteam_count'] :"0"}}
            </label>
          </p>
        </div>
        <div class="gst-bifurcation" style="width: 160px;">
          <p>
            <span>Total GST</span>
            <label>
              {{isset($listDetails['total_gst']) ? $listDetails['total_gst'] :"₹0.0"}}
            </label>
          </p>
          <p>
            <span>CN Amount</span>
            <label>
              {{isset($listDetails['cn_amount']) ? $listDetails['cn_amount'] :"0"}}
            </label>
          </p>
        </div>
        <div class="invoice-total-bifurcation" style="width: 160px;">
          <p>
            <span>Total Amount</span>
            <label>
              {{isset($listDetails['total_amount']) ? $listDetails['total_amount'] :"₹0.0"}}
            </label>
          </p>
          <p>
            <span>Round off.</span>
            <label>{{isset($listDetails['round_off']) ? $listDetails['round_off'] :"₹0.0"}} </label>
          </p>
        </div>
        <div class="net-payable">
          <p class="net-amount">
            ...
            <label>
              {{isset($listDetails['net_amount']) ? $listDetails['net_amount'] :"₹0.0"}}
            </label>
          </p>
        ...
        <div class="branding">
          <ul>
            <li style="display: flex; align-items: center;">
              <!-- <img src="{{ isset($listDetails['logo']) ? $listDetails['logo'] :'https://pharma247.in/public/landing_design/images/logo.png'}}"
                alt="Pharam247" title="Pharam247" class="footer-logo" />Pharma247 -->Thank You !
            </li>
          </ul>
        </div>
      </div>
